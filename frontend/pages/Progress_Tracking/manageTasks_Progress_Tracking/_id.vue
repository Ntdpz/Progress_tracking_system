          ...
          <td>
            <v-tooltip bottom>
              <template v-slot:activator="{ on, attrs }">
                ...
                <v-list-item-avatar v-bind="attrs" v-on="on">
                  <v-img
                    :src="getUserPic(item.task_member_create)"
                    alt="User Avatar"
                  />
                </v-list-item-avatar>
              </template>
          ...
          <td @click.stop>
            ...
            <v-tooltip bottom>
              <template v-slot:activator="{ on, attrs }">
                <v-btn
                  v-if="
                    item.task_member_id === $auth.user.id ||
                    user.user_role === 'Admin'
                  "
                  v-bind="attrs"
                  v-on="on"
...
import task_pic_create from "@/components/Progress_tracking/Dashbord/task_pic_create.vue";
...
export default {
  ...
  methods: {
    ...
    calculateMandays() {
      ...
      if (this.newTask.task_plan_start && this.newTask.task_plan_end) {
        const startDate = new Date(this.newTask.task_plan_start);
        const endDate = new Date(this.newTask.task_plan_end);

        // Calculate business days
        this.newTask.task_manday = this.countBusinessDays(startDate, endDate);
      } else {
    ...
    async fetchUserList() {
      try {
        ...
        if (this.user.user_position === "Developer") {
          // Only show the current user if the position is "Developer"
          ...
          // Show both Developer and Implementer users if the position is "Implementer"
          this.userListCreate = userList
            .filter(
              (user) =>
                user.user_position === "Developer" ||
                user.user_position === "Implementer"
            )
            .map((user) => ({
              user_id: user.id,
              user_name: `${user.user_position} : ${user.user_firstname} ${user.user_lastname}`,
          ...
          // Show all users if the position is not "Developer" or "Implementer"
